...
void Application::initWindow()
{
    ...
    window = glfwCreateWindow(1920, 1080, "Vulkan CPU Physics", nullptr, nullptr);
...
bool Application::isCollidingSphereWithPlane(const PhysicsObject &sphere)
{
    return (sphere.position.y - sphere.radius) <= 0.0f;
}

void Application::resolveCollisionSphereWithPlane(PhysicsObject &sphere)
{
    const float planeFrictionCoefficient = 0.5f;

    sphere.velocity.y = -sphere.velocity.y * sphere.elasticity;
    sphere.position.y = sphere.radius;

    glm::vec3 tangentialVelocity(sphere.velocity.x, 0.0f, sphere.velocity.z);
    glm::vec3 frictionImpulse = -planeFrictionCoefficient * tangentialVelocity;

    sphere.velocity += frictionImpulse;
}

bool Application::isCollidingSphereWithSphere(const PhysicsObject &sphereOne, const PhysicsObject &sphereTwo)
{
    float sumRadiiSquared = (sphereOne.radius + sphereTwo.radius) * (sphereOne.radius + sphereTwo.radius);
    float distanceSquared = glm::dot(sphereTwo.position - sphereOne.position, sphereTwo.position - sphereOne.position);

    return distanceSquared <= sumRadiiSquared;
}

void Application::resolveCollisionSphereWithSphere(PhysicsObject &sphereOne, PhysicsObject &sphereTwo)
{
    glm::vec3 normalDirection = sphereOne.position - sphereTwo.position;
    glm::vec3 normalizedNormalDirection = glm::normalize(normalDirection);

    float separationDistance = glm::length(normalDirection);
    float overlap = (sphereOne.radius + sphereTwo.radius) - separationDistance;

    const float percent = 0.2f;  // Positional correction factor (20%)
    const float slop = 0.00001f; // Allowable penetration
    glm::vec3 correction = std::max(overlap - slop, 0.0f) / (1.0f / sphereOne.mass + 1.0f / sphereTwo.mass) * percent * normalizedNormalDirection;

    sphereOne.position += correction * (1.0f / sphereOne.mass);
    sphereTwo.position -= correction * (1.0f / sphereTwo.mass);

    glm::vec3 relativeVelocity = sphereOne.velocity - sphereTwo.velocity;
    float velocityAlongNormal = glm::dot(relativeVelocity, normalizedNormalDirection);
    ...
    float e = std::min(sphereOne.elasticity, sphereTwo.elasticity);
    float j = -(1.0f + e) * velocityAlongNormal;
    j /= (1.0f / sphereOne.mass) + (1.0f / sphereTwo.mass);

    glm::vec3 impulse = j * normalizedNormalDirection;
...
void Application::updatePhysics(float deltaTime)
{
    ...
    for (size_t i = 0; i < physicsObjects.size(); ++i)
    {
        ...
        // Apply gravity
        physicsObject.velocity += gravity * deltaTime;
        physicsObject.position += physicsObject.velocity * deltaTime;

        // Check for collisions with the plane
        if (isCollidingSphereWithPlane(physicsObject))
        {
            resolveCollisionSphereWithPlane(physicsObject);
        }
        ...
        for (size_t j = 0; j < physicsObjects.size(); ++j)
        {
            if (i != j)
            {
                PhysicsObject &otherPhysicsObject = physicsObjects[j];
                if (isCollidingSphereWithSphere(physicsObject, otherPhysicsObject))
                {
                    resolveCollisionSphereWithSphere(physicsObject, otherPhysicsObject);
                }
            }
        }
    }
    ...
    physicsUpdateTimeMS = duration.count();
}

void Application::updateShaderStorageBuffers()
{
    vk::DeviceSize bufferSize = sizeof(PhysicsObject) * static_cast<uint32_t>(physicsObjects.size());
    ...
    // Update the staging buffer with new physics data
    void *data;
    ...
    // Copy updated data to all storage buffers
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
...
{
    physicsObjects = createSphereBox(PHYSICS_OBJECT_COUNT, 0.115f);
...
void Application::drawOverlay()
{
    ...
    if (ImGui::Begin("Example: Simple overlay", &showWindow, window_flags))
    {
        ...
        ImGui::Separator();
        ImGui::Text("Physics:                   %.3f ms", physicsUpdateTimeMS);
...
void Application::getTimeStampResults()
{
    ...
    graphicsPipelineTimeMS = float(timeStamps[1] - timeStamps[0]) * physicalDeviceLimits.timestampPeriod / 1'000'000.0f;
}